...
namespace Sentinel.Tests.TestHelpers
{
    public static class SentinelTestHelper
    {
        ...
        {
            return new Bitmap(System.IO.Directory.GetFiles("C:\\Work\\FYP\\SentinelFYP\\trunk\\SentinelFYP\\Sentinel\\Content\\etc\\dots", "dot" + zoom + "." + ImageFormat.Png.ToString().ToLower()).First());
        }

        public static Bitmap GetColourSchemeMock(string colourScheme)
        {
            return new Bitmap(System.IO.Directory.GetFiles("C:\\Work\\FYP\\SentinelFYP\\trunk\\SentinelFYP\\Sentinel\\Content\\etc\\color-schemes", colourScheme + "." + ImageFormat.Png.ToString().ToLower()).First());
        }
    }
}
