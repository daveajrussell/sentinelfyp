...
namespace DomainModel.Models.GISModels
{
    ...
    public class Tile
    {
        ...
        /// <returns></returns>
        public static Bitmap Generate(Bitmap colorScheme, Bitmap dot, int zoom,int tileX, int tileY, GMap.NET.Point[] points)
        {
            int expandedWidth;
            int expandedHeight;
            int dotHalfSize;
            ...
            if (points.Length == 0)
            {
                tile = GetEmptyTile(colorScheme, 100);
            }
            else
            {
                ...
                tile = Trim(tile, dot);
                tile = Colorize(tile, colorScheme, 100);
            }
            return tile;
        }
        ...
        {
            Bitmap croppedTile = new Bitmap(SIZE, SIZE, PixelFormat.Format32bppArgb);
        ...
        public static Bitmap GetEmptyTile(Bitmap colorScheme,int zoomOpacity)
        {
            ...
            //Create a blank tile that is 32 bit and has an alpha
            tile = new Bitmap(SIZE, SIZE, System.Drawing.Imaging.PixelFormat.Format32bppArgb);
            ...
            graphic.FillRectangle(solidBrush, 0, 0, SIZE, SIZE);
            graphic.Dispose();
