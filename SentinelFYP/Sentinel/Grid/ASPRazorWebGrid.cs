...
namespace Sentinel.Grid
{
    public class ASPRazorWebGrid
    {
        ...
        public ASPRazorWebGrid(IEnumerable<dynamic> source = null, string gridId = null, bool maintainState = true, bool usePostBack = true, string width = null)
        {
            _source = source;
                ...
                foreach (var item in columns)
                {
                    ...
                    if (_source.Count() > 0 && item.CanSort && (!string.IsNullOrEmpty(item.DataField) || !string.IsNullOrEmpty(item.HeaderText)))
                    {
                            ...
                            && (gridParameters.Sort == item.DataField || gridParameters.Sort == item.SortExpression))
                        {
                            ...
                            img.MergeAttribute("src", "/content/images/" + imgfile);
